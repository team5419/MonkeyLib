...
class CubicBezierSpline(p0: Vector2, p1: Vector2, p2: Vector2, p3: Vector2) : Spline() {
    ...
    public override fun getPoint(t: Double): Vector2 {
        val px =
            Math.pow(1 - t, 3.0) * p0.x +
            3 * Math.pow(1 - t, 2.0) * t * p1.x +
            3 * (1 - t) * Math.pow(t, 2.0) * p2.x +
            Math.pow(t, 3.0) * p3.x

        val py =
            Math.pow(1 - t, 3.0) * p0.y +
            3 * Math.pow(1 - t, 2.0) * t * p1.y +
            3 * (1 - t) * Math.pow(t, 2.0) * p2.y +
            Math.pow(t, 3.0) * p3.y
